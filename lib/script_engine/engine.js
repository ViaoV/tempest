...
class ScriptEngine extends EventEmitter {
  ...
  listScripts() {
    return new Promise((res, rej) => {
      const pattern = path.join(this.scriptsPath, '*.coffee');
      files.listDirectory(pattern, {}, (err, results) => {
        if (err) {
          rej(err);
        } else {
          res(results);
  ...
  listScriptNames() {
    return new Promise((res, rej) => {
      this.listScripts().catch(rej).then((scripts) => {
        res(scripts.map(i => path.basename(i, '.coffee')));
      });
    });
  }
