...
class GameSession extends EventEmitter {
  ...
  popSendBuffer(ignoreRt) {
    ...
    if (!ignoreRt && (this.state.roundTimeEnd + 1) > this.state.time) {
      if (Settings.get('game.roundTimeDelay') === true) {
        return;
      }
    }

    if (this.sendBuffer.length == 0) {
      this.emit('message.empty');
      return;
    }
    ...
    if (Settings.get('localEcho') === true) {
      this.emit('message', {
        stream: 'game',
        text: '> ' + str + '\n',
        style: 'user-cmd',
  ...
  onTagOpen(name, attr) {
    ...
    if (name == 'indicator') {
      ...
      this.state.indicators[indicator] = (attr.visible === 'y');
    }
  ...
  onText(text) {
    ...
    if (!this.insideTag && text.trim() !== '' && this.tagDepth == 0) {
      ...
      if (msg.text.indexOf('...wait') > -1) {
        if (Settings.get('game.commandRequeue') === true) {
